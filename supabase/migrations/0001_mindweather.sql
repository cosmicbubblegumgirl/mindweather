-- MindWeather hosted schema. The runnable app uses localStorage; this migration
-- is the seam for a future Supabase adapter.
...
begin
  foreach table_name in array array[
    'user_preferences','subjects','tasks','task_subtasks','weather_checkins',
    ...
    execute format('create policy "owner access" on public.%I for all using (auth.uid() = user_id) with check (auth.uid() = user_id)', table_name);
  end loop;
end $$;

alter table public.profiles enable row level security;
drop policy if exists "owner access" on public.profiles;
create policy "owner access" on public.profiles for all using (auth.uid() = id) with check (auth.uid() = id);

alter table public.rooms enable row level security;
alter table public.room_members enable row level security;
drop policy if exists "room access" on public.rooms;
create policy "room access" on public.rooms for all using (auth.uid() = owner_id or exists (select 1 from public.room_members m where m.room_id = id and m.user_id = auth.uid())) with check (auth.uid() = owner_id);
drop policy if exists "member access" on public.room_members;
create policy "member access" on public.room_members for all using (auth.uid() = user_id or exists (select 1 from public.rooms r where r.id = room_id and r.owner_id = auth.uid())) with check (auth.uid() = user_id or exists (select 1 from public.rooms r where r.id = room_id and r.owner_id = auth.uid()));
