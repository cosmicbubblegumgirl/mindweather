...
export function JournalView({}: { navigate?(view: ViewId): void }) {
  ...
  return <div className="journal-page">
    ...
    <div className="journal-layout"><section className="journal-entries">{state.journal.map((entry) => <article key={entry.id} className="journal-entry panel"><header><span className="weather-orb" /><div><strong>{entry.prompt}</strong><small>{new Date(entry.createdAt).toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric" })}</small></div><Quote /></header><p>{entry.text}</p><footer>{entry.subjectId && <span style={{ "--tag": state.subjects.find((item) => item.id === entry.subjectId)?.color } as React.CSSProperties}>{state.subjects.find((item) => item.id === entry.subjectId)?.name}</span>}{entry.tags.map((tag) => <em key={tag}>#{tag}</em>)}</footer></article>)}</section><aside className="journal-side panel"><BookHeart /><h3>A small record of real days.</h3><p>Your journal stays inside your protected account and helps only with your own observations.</p><div><strong>{state.journal.length}</strong><span>reflections</span></div><div><strong>{new Set(state.journal.flatMap((entry) => entry.tags)).size}</strong><span>themes noticed</span></div></aside></div>
